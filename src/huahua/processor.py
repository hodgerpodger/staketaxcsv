...
from huahua.constants import CUR_HUAHUA, CURRENCIES, MILLION
from huahua.make_tx import make_huahua_reward_tx, make_transfer_receive_tx
from huahua.TxInfoHuahua import TxInfoHuahua
...
def process_txs(wallet_address, elems, exporter):
    ...
    ErrorCounter.log(TICKER_HUAHUA, wallet_address)
...
def process_tx(wallet_address, elem, exporter):
    ...
    for i in range(0, len(msg_types)):
        ...
        # Make new unique TxInfoHuahua for each message
        cur_txid = "{}-{}".format(txid, i)
        cur_fee = fee if i == 0 else ""
        txinfo = TxInfoHuahua(cur_txid, timestamp, cur_fee, wallet_address, url)
...
def handle_del_reward(exporter, txinfo, elem, msg_index, msg_type):
    ...
    if reward:
        row = make_huahua_reward_tx(txinfo, reward)
        exporter.ingest_row(row)
        return
    ...
    if transfers_in:
        for amount, currency, _, _ in transfers_in:
            row = make_huahua_reward_tx(txinfo, amount)
            exporter.ingest_row(row)
        return
...
def _huahua(uhuahua):
    """
    Example: '5340003uhuahua' -> 5.340003
    """
    amount, currency = _amount(uhuahua)
    assert currency == CUR_HUAHUA
    return amount
...
def _amount(amount_string):
    ...
    if "ibc/" in amount_string:
        ...
        currency = common.ibc_tokens.get_symbol(localconfig, TICKER_HUAHUA, ibc_address)
