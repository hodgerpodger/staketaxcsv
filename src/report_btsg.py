...
def main():
    wallet_address, export_format, txid, options = report_util.parse_args(TICKER_BITSONG)
    ...
    if txid:
        ...
        report_util.run_exports(TICKER_BITSONG, wallet_address, exporter, export_format)
...
def wallet_exists(wallet_address):
    return common.ibc.api_lcd.LcdAPI(BITSONG_NODE).account_exists(wallet_address)


def txone(wallet_address, txid):
    elem = common.ibc.api_lcd.LcdAPI(BITSONG_NODE).get_tx(txid)
...
def estimate_duration(wallet_address):
    ...
    return SECONDS_PER_PAGE * common.ibc.api_lcd.get_txs_pages_count(BITSONG_NODE, wallet_address, max_txs)
...
def txhistory(wallet_address, options):
    ...
    max_txs = localconfig.limit
    progress = ProgressBitsong()
    exporter = Exporter(wallet_address, localconfig, TICKER_BITSONG)

    # Fetch count of transactions to estimate progress more accurately
    count_pages = common.ibc.api_lcd.get_txs_pages_count(BITSONG_NODE, wallet_address, max_txs, debug=localconfig.debug)
