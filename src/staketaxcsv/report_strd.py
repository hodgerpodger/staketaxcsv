...
from staketaxcsv.settings_csv import STRD_NODE, TICKER_STRD, MINTSCAN_ON
...
def _txdata():
    ...
    return TxDataMintscan(TICKER_STRD, max_txs) if MINTSCAN_ON else TxDataLcd(STRD_NODE, max_txs)
...
def balhistory(wallet_address):
    ...
    exporter = historical_balances.via_mintscan(
        STRD_NODE, TICKER_STRD, wallet_address, max_txs, start_date, end_date)
    return exporter
