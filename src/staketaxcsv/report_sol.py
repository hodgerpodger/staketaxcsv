...
def _convert_token_addresses_to_symbols(exporter):
    """ Converts token addresses to ticker symbol in CSV (for those possible) """
    if not SOL_HELIUS_API_KEY:
        logging.warning("Missing SOL_HELIUS_API_KEY in environment.  Need this "
                        "to convert newer token addresses to symbols.")
        return
