...
def get_txs_all(nodes, wallet_address, progress_rpc, max_txs, limit=LIMIT_TXS_PER_QUERY):
    elems = []
    for node in nodes:
        ...
        cur_elems = staketaxcsv.common.ibc.api_rpc.get_txs_all(
            node, wallet_address, progress_rpc, max_txs, limit=limit, stage_name=stage_name_fetch)
