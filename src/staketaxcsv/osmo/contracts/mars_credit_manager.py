...
def handle_credit_manager(exporter, txinfo, msginfo):
    ...
    if "create_credit_account" in msg:
        return _handle_create_credit_account(exporter, txinfo, msginfo, msg)
    elif "update_credit_account" in msg:
        return _handle_update_credit_account(exporter, txinfo, msginfo, msg)

    raise Exception("Unable to handle tx in mars_credit_manager.handle_credit_manager()")


def _handle_create_credit_account(exporter, txinfo, msginfo, msg):
    transfers_in, transfers_out = msginfo.transfers
...
def _handle_update_credit_account(exporter, txinfo, msginfo, msg):
    transfers_in, transfers_out = msginfo.transfers
