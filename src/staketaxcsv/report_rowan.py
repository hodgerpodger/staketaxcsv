...
def wallet_exists(wallet_address):
    return staketaxcsv.common.ibc.api_lcd.LcdAPI_v1(ROWAN_NODE).account_exists(wallet_address)


def txone(wallet_address, txid):
    elem = staketaxcsv.common.ibc.api_lcd.LcdAPI_v1(ROWAN_NODE).get_tx(txid)
...
def estimate_duration(wallet_address):
    ...
    return SECONDS_PER_PAGE * staketaxcsv.common.ibc.api_lcd.get_txs_pages_count(ROWAN_NODE, wallet_address, max_txs)
...
def txhistory(wallet_address):
    ...
    count_pages = staketaxcsv.common.ibc.api_lcd.get_txs_pages_count(ROWAN_NODE, wallet_address, max_txs, debug=localconfig.debug)
    progress.set_estimate(count_pages)

    # Fetch transactions
    elems = staketaxcsv.common.ibc.api_lcd.get_txs_all(ROWAN_NODE, wallet_address, progress, max_txs, debug=localconfig.debug)
